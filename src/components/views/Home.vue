      ...
      <template v-if="this.vacuumState.connected">
        ...
        <section class="robot-control-buttons">
          <v-ons-button
            ...
            :key="button.command"
            class="button-margin"
            @click="handleControlButton(button.command)"
        ...
        <section style="margin: 10px 16px">
          ...
          <p>
            <v-ons-progress-bar
              id="battery-status-bar"
              :value="this.vacuumState.battery"
              secondary-value="100"
            ></v-ons-progress-bar>
          </p>
        </section>
...
export default class Home extends Vue {
  ...
  public setButtonState() {
    if (this.vacuumState.connected) {
      this.buttons
        .filter(
          button => button.command !== "goto" && button.command !== "area"
        )
    ...
    fanPowerButton.label =
      FanSpeeds[this.vacuumState.fan_power] ||
      `Fan Power: ${this.vacuumState.fan_power}%`;
  }
  ...
  public async handleControlButton(command: string) {
    ...
    const speeds = Object.keys(FanSpeeds).filter(
      key => typeof FanSpeeds[key as any] === "number"
    );
