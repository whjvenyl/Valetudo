  ...
  <v-ons-page>
    <div class="content">
      ...
      <v-ons-list id="zones-list">
        <v-ons-list-item v-if="zones.length === 0 && !fetching">No zones are configured yet.</v-ons-list-item>
        <v-ons-list-item v-if="fetching">Loading...</v-ons-list-item>
        <v-ons-list-item tappable="true" v-for="(zone, key) of zones" :key="zone.name">
          <label class="left">
            <v-ons-checkbox :input-id="'zones-index'+key" @click="toggleSelect(zone)"
                            :checked="isSelected(zone) > -1"></v-ons-checkbox>
          </label>
        ...
        <v-ons-list-item>
          <div class="center">
            <v-ons-button modifier="large" :disabled="fetching" class="button-margin" @click="handleZonesCleanButton">
              Clean zone
            </v-ons-button>
          </div>
        </v-ons-list-item>
